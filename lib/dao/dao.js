...
function isWhereByGivenId(Model, where, idValues) {
    if ((Object.getOwnPropertyNames(where).length === 0) || (Object.getOwnPropertyNames(idValues).length === 0)) {
        throw new err("isWhereByGivenId(Model, where, idValues) Called with empty arguments ((model)%s, %s, %s)", Model.modelName, JSON.stringify(where), JSON.stringify(idValues));
    }
