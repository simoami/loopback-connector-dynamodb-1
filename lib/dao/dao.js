...
function getIdValues(m, data) {
    var idValues = {};
    var idNames = m.definition.idNames();
    idNames.forEach(function(id) {
        idValues[id] = data[id];
    });
    //debug("Names: %s", idNames);
    //debug("Values: %s", JSON.stringify(idValues));
    return idValues;
}
...
function byIdQuery(m, idValues) {
    var query = {
        where: {}
    };
    Object.keys(idValues).forEach(function(key) {
        query.where[key] = idValues[key];
    });

    return query;
}

function isWhereByGivenId(Model, where, idValues) {
    if ((Object.getOwnPropertyNames(where).length === 0) || (Object.getOwnPropertyNames(idValues).length === 0)) {
        throw new Error("isWhereByGivenId(Model, where, idValues) Called with empty arguments ((model)%s, %s, %s)", Model.modelName, JSON.stringify(where), JSON.stringify(idValues));
    }
    debug("isWhereByGivenId(Model, where, idValues) Called with ((model)%s, %s, %s)", Model.modelName, where, idValues);

    var idMatches = false;
    var numKeys = Object.keys(idValues).length;
    // If only 1 ID exists, it must be a hash key.
    if (numKeys === 1) idMatches = (where[Model.hashKeyName] === idValues[Model.hashKeyName]);
    // Sort key exists
    else if (numKeys === 2) idMatches = ((where[Model.hashKeyName] === idValues[Model.hashKeyName]) && (where[Model.sortKeyName] === idValues[Model.sortKeyName]));
 ...
 *  debug(user instanceof User); // true
 * });
 * ```
...
DataAccessObject.create = function(data, options, cb) {
    ...
    var enforced = {};
    var obj;
    var idValues = getIdValues(this, data);

    // if we come from save
    if (data instanceof Model && !idValues) {
        obj = data;
    } else {
        obj = new Model(data);
    }
    ...
    function create() {
        obj.trigger("create", function(createDone) {
            obj.trigger("save", function(saveDone) {
                ...
                debug("CONTEXT: %s", JSON.stringify(context));
                debug("connector.create.length: %d", connector.create.length);
                debug("connector.create %s", connector.create);
...
DataAccessObject.updateOrCreate =
    ...
    DataAccessObject.upsert = function(data, options, cb) {
        ...
        // create set of idvalues 
        this.definition.idNames().forEach(function(id) {
            idValues[id] = data[id];
            debug("idValues[%s] === %s", id, data[id]);
        });
        

        if (Object.keys(idValues).length === 0 && idValues.constructor === Object) {
            cb(new Error("ID Values not found"));
            //return this.create(data, options, cb);
        }
        else if (Object.keys(idValues).length === 1 && idValues.constructor === Object) {
            // only hashKey.
            if (data[idValues[0]] === undefined) {
                // TODO: Create UUID
                cb(new Error("This should create a new ID - but is not yet implemented!"));
            }

        }
        debug("byIdQuery((Model)%s,%s) => %s", Model.modelName, JSON.stringify(idValues), JSON.stringify(byIdQuery(Model, idValues)));
        var context = {
            Model: Model,
            query: byIdQuery(Model, idValues),
            hookState: hookState,
            options: options,
            data: data
        };
        ...
        function doUpdateOrCreate(err, ctx) {
            ...
            var isOriginalQuery = isWhereByGivenId(Model, ctx.query.where, idValues);
            ...
            if (connector.updateOrCreate && isOriginalQuery) {
                ...
                debug("Calling findOne(where, opts, cb) => findOne(%s, %s, cb)", JSON.stringify(ctx.query.where), JSON.stringify(opts));
                ...
                Model.findOne({
                    ...
                    if (!isOriginalQuery) {
                        ...
                        debug("using original query");
                    }
                    if (inst) {
                        debug("INST Data: %s", JSON.stringify(data));
...
DataAccessObject.replaceOrCreate = function replaceOrCreate(data, options, cb) {
    ...
    var context = {
        Model: Model,
        query: byIdQuery(Model, getIdValues(Model, inst)),
        hookState: hookState,
        options: options,
    };
    ...
    function doReplaceOrCreate(err, ctx) {
        if (err) return cb(err);
        var isOriginalQuery = isWhereByGivenId(Model, ctx.query.where, ids);
        ...
        if (connector.replaceOrCreate && isOriginalQuery) {
            ...
            Model.findOne({
                ...
                if (!isOriginalQuery) {
                    // The custom query returned from a hook may hide the fact that
                    // there is already a model with `id` value `data[idName(Model)]`
                    var pkNames = this.idNames(Model);
                    if (pkNames.length === 2) {
                        delete data[pkNames[0]];
                        delete data[pkNames[1]];
                    }
                    else if (pkNames.length === 1) {
                        delete data[pkNames[0]];
                    }
                    if (found) ids = found[pkName];
                }
                if (found) {
                    self.replaceById(ids, data, options, cb);
                } else {
                    Model = self.lookupModel(data);
                    var obj = new Model(data);
                    obj.save(options, cb);
 ...
 * @param {hashKey} hashKey Identifier of object (primary key value)
 * @param {sortKey} sortKey Identifier of object (primary key value)
 * @param {Object} [options] Options
 * @param {Function} cb Callback function called with (err, exists: Bool)
 */
DataAccessObject.exists = function exists(hashKey, sortKey, options, cb) {
    var connectionPromise = stillConnecting(this.getDataSource(), this, arguments);
    if (connectionPromise) {
        return connectionPromise;
    }

    assert(arguments.length >= 2, "The hashKey and sortKey arguments are required");
    if (cb === undefined) {
        if (typeof options === "function") {
            // exists(id, cb)
            cb = options;
    ...
    var idNames = this.definition.idNames();
    var id = {};
    id[idNames[0]] = hashKey;
    id[idNames[1]] = sortKey;
 ...
 */
DataAccessObject.findById = function findById(hashkey, sortkey, filter, options, cb) {
    //debug("FindById:\n\tHK:%s\n\tSK:%s\n\tFilter:%s\n\tOptions:%s\n\tCallback:%s", hashkey, JSON.stringify(sortkey), JSON.stringify(filter), JSON.stringify(options), cb)
    var connectionPromise = stillConnecting(this.getDataSource(), this, arguments);
    if (connectionPromise) {
        return connectionPromise;
    }

    assert(arguments.length >= 1, "The hashkey argument is required");
    
    
    if (filter === undefined && options === undefined && cb === undefined) {
        ...
        if (typeof options === "function") {
            cb = options;
            options = {};
            if (typeof filter === "object" && !(filter.where || filter.include || filter.fields)) { 
                options = filter;
                filter = {};
            }
        }
    ...
    // assert(typeof sortkey === "string", "The sortkey arguement must be a string");
    ...
    if (isPKMissing(this, cb)) {
        return cb.promise;
    } else if (hashkey == null || hashkey === "") {
        process.nextTick(function() {
            cb(new Error(g.f("{{Model::findById}} requires the {{hashkey}} argument")));
        });
    } else {
        ...
        if (sortkey === undefined) {
            ...
            query.where[idNames[0]] = hashkey;
            this.find(query, options, cb);
        } else {
            
            ids[idNames[1]] = sortkey;
...
DataAccessObject.find = function find(query, options, cb) {
    ...
    assert(typeof options === "object", "The options argument must be an object");
    assert(typeof cb === "function", "The cb argument must be a function");
    debug("Query: %s\nOptions:%s", JSON.stringify(query), JSON.stringify(options));
    var hookState = {};
    var self = this;
    var connector = self.getConnector();
...
DataAccessObject.removeById =
    ...
    DataAccessObject.deleteById = function deleteById(id, options, cb) {
        ...
        this.remove(byIdQuthisery(this, id).where, options, function(err, info) {
            if (err) return cb(err);
            ...
            inst.trigger("save", function(saveDone) {
                inst.trigger("update", function(updateDone) {
                    ...
                    context = {
                        Model: Model,
                        data: data,
                        where: byIdQuery(Model, getIdValues(Model, inst)).where,
                        currentInstance: inst,
                        hookState: hookState,
                        options: options,
                    };
...
DataAccessObject.update =
    DataAccessObject.updateAll = function(where, data, options, cb) {
        ...
        debug("Applied Scope & Properties");
        where = query.where;
        ...
        function doUpdate(where, data) {
            ...
            debug("Update Query Arguements: \n\tModel Name: %s \n\tWhere: %s\n\tData: %s\n\tOptions: %s\n\tCallback: %s", Model.modelName, where, data, options, updateCallback);
...
DataAccessObject.prototype.remove =
    ...
    DataAccessObject.prototype.destroy = function(options, cb) {
        ...
        var Model = this.constructor;
        var id = getIdValues(this.constructor, this);
        var hookState = {};
...
DataAccessObject.replaceById = function(id, data, options, cb) {
    ...
    if (id !== data[pkName]) {
        var err = new Error(g.f("{{id}} cannot be updated from " +
            "%s to %s when forceId is set to true", inst[pkName], data[pkName]));
        err.statusCode = 400;
        process.nextTick(function() {
            cb(err);
        });
...
DataAccessObject.updateAttributes =
    DataAccessObject.patchAttributes =
    function(hashkey, sortkey, data, options, cb) {
        ...
        Object.keys(idNames).forEach(function(idName) {
        ...
        //debug("Instance: %s", JSON.stringify(inst));
        var context = {
            Model: this,
            where: byIdQuery(this, getIdValues(this, inst)).where,
            data: data,
            currentInstance: inst,
            hookState: hookState,
            options: options,
        ...
        this.notifyObserversOf("before save", context, function(err, ctx) {
            if (err) return cb(err);
            data = ctx.data;
            ...
            function validateAndSave(err, data) {
                ...
                var doValidate = true;
                if (options.validate === undefined) {
                    if (self.settings.automaticValidation !== undefined) {
                        doValidate = this.settings.automaticValidation;
                    }
                } else {
                    doValidate = options.validate;
                }
                    ...
                    inst.trigger("save", function(saveDone) {
                        inst.trigger("update", function(done) {
                            ...
                            function updateAttributesCallback(err) {
                                ...
                                this.notifyObserversOf("loaded", ctx, function(err) {
                                    if (err) return cb(err);
                                    ...
                                    if (this.settings.updateOnLoad) {
                                        ...
                                        saveDone.call(inst, function() {
                                            ...
                                            this.notifyObserversOf("after save", context, function(err) {
                                                cb(err, inst);
                                            });
                                        });
                                    });
                            ...
                            this.notifyObserversOf("persist", ctx, function(err) {
                                if (connector.updateAttributes.length === 5) {
                                    connector.updateAttributes(model, getIdValues(inst.constructor, inst),
...
DataAccessObject.prototype.reload = function reload(cb) {
    ...
    return this.constructor.findById(getIdValues(this.constructor, this), cb);
};
