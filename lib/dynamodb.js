...
DynamoDB.prototype.define = function (descr) {
    ...
    for (var key in properties) {
        ...
        if (properties[key].keyType !== undefined) {
            ...
            // This never gets called if ID injection is true
            if (attrs.keyType === "pk") {
                this._models[modelName].pKey = key;
                logger.log("debug", "PRIMARY KEY:", key);
    ...
    if ((this._models[modelName].rangeKey !== undefined) && (this._models[modelName].pKey === undefined && (descr.idInjection === false))) {
...
DynamoDB.prototype.all = function all(model, filter, callback) {
    ...
    // check for pKey, generate if only hash found.
    ...
    if (hashKeyFound === true) {
        ...
        if (this._models[model].rangeKey === undefined) {
            var pKey = String(this._models[model].hashKey) + '--x--' + String(helper.UUID());
        } else {
            var pkSeparator = this._models[model].pkSeparator;
            var pKey = String(this._models[model].hashKey) + pkSeparator + String(this._models[model].rangeKey);
            var rangeKey = this._models[model].rangeKey;
        }
