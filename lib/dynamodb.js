...
DynamoDB.prototype.create = function(model, data, callback) {
    ...
    debug("data sent to create: %s", data);
...
DynamoDB.prototype.updateAttributes = function(model, options, data, callback) {
    var timeStart = startTimer();
    ...
    if ((sortKey !== undefined) && ((options[sortKey] === null) || (options[sortKey] === undefined))) {
        var err = new Error("Sort Key `" + sortKey + "` cannot be null or undefined.");
        callback(err, null);
        return;
    }
    //debug("HK=%s SK=%s", options[hashKey], options[sortKey]);
    // Log queryString
    ...
    if (sortKey !== undefined) {
        tableParams.Key[this._models[model].hashKey] = options[hashKey];
        tableParams.Key[this._models[model].sortKey] = options[sortKey];
    } else {
        tableParams.Key[this._models[model].hashKey] = options[hashKey];
    }
