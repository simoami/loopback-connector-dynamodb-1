...
DynamoDB.prototype.define = function(descr) {
    ...
    for (var key in properties) {
        ...
        if (properties[key].keyType !== undefined) {
            ...
            logger.log("debug", "CHECKING:", attrs.keyType);
 ...
 * @param  {object/primitive}   pKey   : If sort key is undefined,
 *                                       this is the same as hash key. If sort key is defined, 
 *                                       then pKey is hashKey + (Separator) + sortKey
 * @param  {Function} callback 
 */
DynamoDB.prototype.find = function find(model, hk, callback) {
    var timeStart = startTimer();
    var queryString = "GET ITEMS BY HASH KEY FROM TABLE ";
    var hashKey = this._models[model].hashKey;
    var attributeSpecs = this._attributeSpecs[model];
...
/**
 * Find an item based on hashKey & sortKey
 * @param  {object}   model    [description]
 * @param  {object/primitive}   pKey   : If sort key is undefined,
 *                                       this is the same as hash key. If sort key is defined, 
 *                                       then pKey is hashKey + (Separator) + sortKey
 * @param  {Function} callback 
 */
DynamoDB.prototype.findByIds = function findOne(model, pk, callback) {
    var timeStart = startTimer();
    var queryString = "GET ITEM BY HASH AND SORT KEYS FROM TABLE ";
    var hashKey = this._models[model].hashKey;
    var sortKey = this._models[model].sortKey;
    var attributeSpecs = this._attributeSpecs[model];
    var hk, sk;
...
DynamoDB.prototype.updateAttributes = function(model, pk, data, callback) {
    ...
    if (pKey !== undefined) {
        if ((data[sortKey] === null) || (data[sortKey] === undefined)) {
            err = new Error("Sort Key `" + sortKey + "` cannot be null or undefined.");
            callback(err, null);
            return;
        } else {
            data[pKey] = String(data[hashKey]) + pkSeparator + String(data[sortKey]);
        ...
        if (err) {
            ...
            async.mapSeries(res.Items, function(item, insideCallback) {
                ...
                if (sortKey === undefined) {
                    ...
                    hk = item[hashKey];
                    sk = item[sortKey];
                    pk = String(hk) + pkSeparator + String(sk);
                }
