 ...
 *
 */
DynamoDB.prototype.all = function all(model, filter, options, callback) {
    var timeStart = startTimer();
...
DynamoDB.prototype.update =
    DynamoDB.prototype.updateAll = function updateAll(model, where, data, options, cb) {
        ...
        this.all(model, filter, options, function(err, results) {
            if (err || !results) {
                cb(err, null);
            } else {
                results.forEach((inst) => {
                    ...
                    self.updateAttributes(model, instIds, data, options, cb);
...
DynamoDB.prototype.exists = function exists(model, ids, callback) {
    ...
    this.all(model, query, options, function(err, record) {
        if (err) {
            callback(err, null);
